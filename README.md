...
| Static: checksum | installed bytes compared with generator output byte for byte, a symlinked destination rejected rather than followed, root-UUID fallback compare, `.ry.bak` copies in `~/ry-install/backups/` non-empty |
